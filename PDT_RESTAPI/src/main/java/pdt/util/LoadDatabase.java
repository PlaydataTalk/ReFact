...
public class LoadDatabase {
	...
	CommandLineRunner initDatabase1(UserRepository repository) {
		...
		return args -> {
			...
			log.info("Preloading " + repository.save(User.builder().userEmail("abc@gmail.com").userName("유재석").userImgUrl("MrYou.png").build()));
			log.info("Preloading " + repository.save(User.builder().userEmail("efg@naver.com").userName("백종원").userImgUrl("MrBaek.jpg").build()));
		};
	}
